...
// Corey added these
let urlInput = "https://bitmidi.com/uploads/85263.mid";
let trainingStep = 0; 
let trainingSteps = 0;
let totalSteps; 
...
function ready(mode) {
  ...
  for (let i = 0; i < N_DIMS; i++) { //CF: this updates sliders an tries to display so might be important
    ...
    for (let i = 0; i < 4; i++) { //made this myself to create the midi me sliders
    const div = document.createElement('div');
    div.classList.add('range-wrap');
    div.innerHTML = `<input type="range" data-index=${i} min="-2" max="2" step="0.1" value="0">`;
    midimeSliders.appendChild(div);
  }
}
...
// function loadSample() {
//   console.log("file-loading");//updateUI('file-loading');
...
//  updateUI('file-loading');
  ...
  mm.urlToBlob(urlInput).then((blob) => {
...
async function showInput(ns) {
  ...
  if (isMelodyMode && shouldSplit) {
    ...
    console.log("has-melody");// updateUI('has-melody');
  } else {
  ...
  console.log("file-loaded");//updateUI('file-loaded');
...
async function sample() {
  ...
  if (midime && midime.trained) {
    console.log("midi me is trained - what the h");
    // If we've trained, then we sample from MidiMe.
    const s = await midime.sample(1);
    zArray = s.arraySync()[0];
    currentSample = (await mvae.decode(s))[0]; //< this is the current note thing
    console.log(currentSample);
    playerMelody.start(currentSample);
    ...
    console.log("midi me not trained");
...
function updateCopy() {
  trainingSteps = 100; //(isMelodyMode ? 100 : 300);
}
